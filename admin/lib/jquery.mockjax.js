...
(function (root, factory) {
	...
	function getMockForRequest(handler, requestSettings) {
		...
		if ($.isFunction(handler.url.test)) {
			...
			var ns = handler.namespace || $.mockjaxSettings.namespace;
			if (ns) {
				var rqUrl = requestSettings.url.replace(ns, '');
			}
			// The user provided a regex for the url, test it
			if (!handler.url.test(rqUrl)) {
				return null;
			}
		} else {
			...
			if (!!namespace) {
				...
				handler.url = handler.url.replace(namespace, '');

				var namespacedUrl = [namespace, handler.url].join('/');
				var proto = '';
				if ((/^http:|https:/gi).test(namespacedUrl)) {
					proto = namespacedUrl.replace(/(^http:\/\/|https:\/\/).*(\w*)/gi, '$1');
					namespacedUrl = namespacedUrl.substr(proto.length);
				}
				namespacedUrl = namespacedUrl.replace(/(\/+)/g, '/');
				handler.url = proto + namespacedUrl;
			}

			// Look for a simple wildcard '*' or a direct URL match
			var star = handler.url.indexOf('*');
			var urlSets = requestSettings.url.split('?'), url = urlSets[0], pms = urlSets[1];
			if (handler.url !== url && star === -1 || !new RegExp(handler.url.replace(/[-[\]{}()+?.,\\^$|#\s]/g, '\\$&').replace(/\*/g, '.+')).test(url)) {
				return null;
			}
		}
